...
export class UserLocationListener {
	...
	// private distanceBound: number = Infinity; // for testing ONLY
	...
	private showCanvasDistanceError = $derived<boolean>(
		!this.canvasEditAccess && this.hasValidDistance
	); // is user too far from the canvas with a valid distance?
	...
	constructor(canvasLatitude: number, canvasLongitude: number) {
	...
	getDistance() {
		...
		if (this.distances.length > 0)
			return (
				this.distances.reduce((runningSum, next) => runningSum + next) /
				this.distances.length
			);
		else return Infinity; // indicates no distance has been recorded yet (e.g. location of user cannot be found)
	}
	...
	pushDistanceFromCanvas(userLat: number, userLon: number, canvasLat: number, canvasLon: number) {
		const currDistance: number = calculateDistanceBetweenCoordinates(
			userLat,
			userLon,
			canvasLat,
			canvasLon
	...
	async setupListener() {
		...
		if (error !== -1) {
			...
			this.listenerID = navigator.geolocation.watchPosition(
				(position) => {
					this.pushDistanceFromCanvas(
						position.coords.latitude,
						position.coords.longitude,
						this.canvasLatitude,
						this.canvasLongitude
					);
